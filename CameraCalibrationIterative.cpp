...
Point2f getIntersectionPoint(int x, int y, vector<Point2f>& new_points) {
	...
	int y_a = offset - y * cols;

	for (int i = 0; i < 4; ++i) {
		int offset = y_a + i * cols;
		pointsINLINE2.push_back(new_points[offset]);
	}


	fitLine(pointsINLINE1, _line, CV_DIST_L2,  0, 0.01, 0.01);
	Point2f p1 (_line[2], _line[3]), p2 ;

	p2.x = p1.x + 205 * _line[0];
	p2.y = p1.y + 205 * _line[1];

	fitLine(pointsINLINE2, _line, CV_DIST_L2,  0, 0.01, 0.01);
...
bool select_frames_process(VideoCapture &cap, int w, int h, const int n_frames, vector<int> &frames, const int n_rows, const int n_columns, Mat &m_calibration, Mat &m_centroids, const Mat camera_matrix, const Mat dist_coeffs) {
	...
	cout << endl << "Start selection with " << selected_frames << " of " << n_frames << endl;
